...
class MdnsSearchDevices {
  Future<void> testMinimalCode() async {
    List<Future<List<String>>> activeHostListsFuture = [];

    for (String serviceType in tcpSrvRecordsList) {
      activeHostListsFuture.add(_findingMdnsWithAddress(serviceType));
    }

    for (final Future<List<String>> activeHostListFuture
        in activeHostListsFuture) {
      await activeHostListFuture;
    }
    return;
  }

  Future<List<String>> _findingMdnsWithAddress(
    String serviceType,
  ) async {
    final List<String> mdnsFoundList = [];
    ...
    await for (final PtrResourceRecord ptr in client.lookup<PtrResourceRecord>(
      ...
      await for (final SrvResourceRecord srv
        ...
        mdnsFoundList.add(srv.toString());
      }
    }
    client.stop();
    return mdnsFoundList;
  }
}
